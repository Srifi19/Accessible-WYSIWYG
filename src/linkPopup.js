...
export class LinkPopup {
  ...
  _build(container) {
    ...
    this._el.innerHTML = `
      <div class="wysiwyg-link-popup-content">
        ...
        <input
          ...
          autocomplete="url"
        />
        <span id="wysiwyg-link-url-error"
              class="wysiwyg-field-error"
        ...
        <input
          ...
          autocomplete="off"
        />
        <span id="wysiwyg-link-display-error"
              class="wysiwyg-field-error"
              aria-live="polite"></span>
    ...
    this._liveRegion = container.querySelector('#wysiwyg-sr-announce');
    if (!this._liveRegion) {
  ...
  _bind() {
    this._el.querySelector('.wysiwyg-link-apply')
      .addEventListener('click', () => this.apply());

    this._el.querySelector('.wysiwyg-link-cancel')
  ...
  _disableBackground() {
    this._inertEls = Array.from(this._container.children)
      .filter(el => el !== this._el);
  ...
  open(openerBtn = null) {
    ...
    document.addEventListener('focusin', this._trapFocus, true);
  ...
  close() {
    ...
    document.removeEventListener('focusin', this._trapFocus, true);
    document.body.classList.remove('wysiwyg-no-scroll');

    const opener = this._opener;
    this._opener = null;

    opener?.focus?.() || this._getEditor()?.commands.focus();
  }
  ...
  apply() {
    ...
    if (!rawUrl) {
      ...
      firstInvalid ||= this._urlInput;
    } else if (!isValidUrl(addProtocol(rawUrl))) {
    ...
    const success = editor.chain().focus()
      .insertContent(`<a href="${addProtocol(rawUrl)}">${rawLabel}</a>`)
      .run();
